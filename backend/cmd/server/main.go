...
func main() {
	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.AuthMiddleware(authService, userRepo, apiKeyRepo))
		{
			...
			protected.PUT("/users/:id", middleware.RBACMiddleware(userRepo, "users", "WRITE"), userHandler.UpdateUser)
			protected.DELETE("/users/:id", middleware.RBACMiddleware(userRepo, "users", "DELETE"), userHandler.DeleteUser)
