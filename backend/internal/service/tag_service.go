...
func (s *TagService) ListGlobalPaginated(limit, offset int, searchTerm string, user *domain.User) ([]domain.Tag, int64, error) {
	scope := domain.GetPermissionScope(user, "tags", "READ")
	return s.repo.ListGlobalPaginated(limit, offset, searchTerm, &scope)
}
