...
func (s *WorkflowService) ListGlobalPaginated(limit, offset int, searchTerm string, tagIDs []uuid.UUID, isTemplate *bool, scope *domain.PermissionScope) ([]domain.Workflow, int64, error) {
	return s.repo.ListGlobalPaginated(limit, offset, searchTerm, tagIDs, isTemplate, scope)
}
...
func (s *WorkflowService) ImportWorkflow(wf *domain.Workflow, user *domain.User) error {
	s.resetWorkflowIDs(wf)
	return s.CreateWorkflow(wf, user)
}

func (s *WorkflowService) ImportUpdateWorkflow(targetID uuid.UUID, wf *domain.Workflow, user *domain.User) error {
	s.resetWorkflowIDs(wf)
	wf.ID = targetID
	return s.UpdateWorkflow(wf, user)
}

func (s *WorkflowService) resetWorkflowIDs(wf *domain.Workflow) {
	wf.ID = uuid.Nil
	for i := range wf.Inputs {
