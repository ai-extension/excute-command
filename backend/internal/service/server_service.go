...
// ExecuteCommandWithTTY chạy lệnh trong PTY session, hỗ trợ auto-input qua stdinCh.
// Khi stdinCh != nil, goroutine caller tự match pattern và send vào channel này.
func (s *ServerService) ExecuteCommandWithTTY(ctx context.Context, serverID uuid.UUID, commandText string, stdinCh <-chan string, user *domain.User, writers ...io.Writer) (string, error) {
	conn, _, err := s.getConnection(ctx, serverID, user, "EXECUTE")
	if err != nil {
		return "", err
	}
	...
	return conn.ExecuteWithTTY(ctx, commandText, stdinCh, writers...)
}
...
func (s *ServerService) ExecuteHttp(ctx context.Context, serverID uuid.UUID, method, url, body, headersStr string, user *domain.User, logWriter io.Writer) (string, error) {
	...
	if hasCurl {
		...
		for k, v := range headers {
			curlCmd += fmt.Sprintf(" -H %s", shellQuote(fmt.Sprintf("%s: %s", k, v)))
		}
		if body != "" {
			curlCmd += fmt.Sprintf(" -d %s", shellQuote(body))
		}
		curlCmd += fmt.Sprintf(" %s", shellQuote(url))
	...
	httpgetCmd := fmt.Sprintf("chmod +x %s 2>/dev/null; %s -u %s -X %s -H %s",
		remoteBinary, remoteBinary, shellQuote(url), shellQuote(method), shellQuote(headersStr))
	if body != "" {
		httpgetCmd += fmt.Sprintf(" -d %s", shellQuote(body))
	}
