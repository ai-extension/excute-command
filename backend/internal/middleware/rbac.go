...
func RBACMiddleware(db *gorm.DB, userRepo domain.UserRepository, permType, action string) gin.HandlerFunc {
	return RBACMiddlewareAny(db, userRepo, permType, action)
}

func RBACMiddlewareAny(db *gorm.DB, userRepo domain.UserRepository, permType string, actions ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
