...
func (h *PermissionHandler) ListResourceItems(c *gin.Context) {
	...
	case "workflows":
		items, total, err = h.workflowRepo.ListGlobalPaginated(limit, offset, searchTerm, nil, nil, &scope)
	...
	case "servers":
		items, total, err = h.serverRepo.ListPaginated(nil, limit, offset, searchTerm, "", nil, nil, &scope)
