...
function ResourceCategoryRow({ category, activeNamespaceId, isPermEnabled, togglePermission, rolePerms }: any) {
    ...
    const [explicitlyAddedIds, setExplicitlyAddedIds] = useState<string[]>([]);

    const getPermByAction = (action: string) => category.permissions.find((p: any) => p.action === action);

    // Initialize explicitlyAddedIds from rolePerms on mount
    useEffect(() => {
        ...
        if (ids.size > 0) {
            setExplicitlyAddedIds(prev => {
                const newIds = new Set([...prev, ...Array.from(ids)]);
                return Array.from(newIds);
            });
        }
    }, [category.permissions]); // Re-run if category perms change (e.g. category switch)

    const addedItemsIds = useMemo(() => {
        const ids = new Set<string>(explicitlyAddedIds);
        rolePerms.forEach((rp: any) => {
            const pId = rp.permission_id || rp.permission?.id;
            const isInCategory = category.permissions.some((p: any) => p.id === pId);
            if (isInCategory && rp.resource_id) {
    ...
    }, [rolePerms, category.permissions, explicitlyAddedIds]);
